...
bool IntelPlugin::CompressToScratchImage(ScratchImage **scrImageScratch_, ScratchImage **scrUncompressedImageScratch_, bool hasAlpha_)
{
	...
	{
		// When dithering is enabled for BC1/BC3, use DirectXTex Compress() instead of ISPC
		if (ps.data->useDithering &&
			(ps.data->encoding_g == DXGI_FORMAT_BC1_UNORM ||
			 ps.data->encoding_g == DXGI_FORMAT_BC3_UNORM))
		{
			DWORD flags = TEX_COMPRESS_DITHER;
...
{  
	...
	//Decompress scrImageScratch image into an uncompressed one
