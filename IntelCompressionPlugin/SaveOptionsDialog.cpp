...
void OptionsDialog::Init(void)
{
	...
	PICheckBox ditheringCheck = GetItem(IDC_DITHERING_CHECK);
	ditheringCheck.SetChecked(mDialogData.UseDithering);

	// Initialize error metric combo (Perceptual / Uniform)
	PIComboBox errorMetricCombo = GetItem(IDC_ERRORMETRIC_COMBO);
	errorMetricCombo.Clear();
	errorMetricCombo.AppendItem("Perceptual");
	errorMetricCombo.AppendItem("Uniform");
	SendMessage(errorMetricCombo.GetItem(), CB_SETCURSEL, mDialogData.UseUniformMetric ? 1 : 0, 0);
...
void OptionsDialog::SetUIFromData()
{
	...
	PICheckBox DitheringCheckBox = GetItem(IDC_DITHERING_CHECK);
	DitheringCheckBox.SetChecked(mDialogData.UseDithering);
...
void OptionsDialog::ExtractDataFromUI(DialogData & dd)
{
	...
	PICheckBox DitheringCheck = GetItem(IDC_DITHERING_CHECK);
	dd.UseDithering = DitheringCheck.GetChecked();
...
void OptionsDialog::DisableUnavailableControls()
{
	...
	{
		::EnableWindow(DitheringCheck.GetItem(), false);
		DitheringCheck.SetChecked(false);
		::EnableWindow(ErrorMetricCombo.GetItem(), false);
	}
	else
	{
		::EnableWindow(DitheringCheck.GetItem(), true);
		::EnableWindow(ErrorMetricCombo.GetItem(), true);
	}
...
vector<string> GetCompressionHelpText(void)
{
	...
	// Help Text
	vs.push_back("Specify the compression settings for the texture.\n\n\
BC1 (DXT1):  Uses 4 bits per pixel.  Contains RGB data with optional 1-bit alpha.  Good balance of quality and file size for color maps and normal maps.\n\n\
BC3 (DXT5):  Uses 8 bits per pixel.  Contains RGBA data with full 8-bit alpha channel.  Use for textures that require alpha transparency.\n\n\
Uncompressed (BGRA8):  Uses 32 bits per pixel.  Lossless, no compression applied.  Largest file size but highest quality.");
...
Color:  Export only the RGB part of the image. Use for images without alpha or when alpha is not wanted.\n\n\
